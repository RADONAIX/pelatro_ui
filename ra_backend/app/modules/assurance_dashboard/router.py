...
from app.modules.assurance_dashboard import schemas, service
...
async def get_dashboard(
    ...
    # Only Rating has a canonical results table. The others 404 rather than
    # returning an all-zero dashboard, which a caller could not tell apart from
    # a real assurance that happens to have found nothing.
    if assurance.strip().lower() != service.SUPPORTED_ASSURANCE:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"No dashboard data source for assurance '{assurance}'",
        )
    return schemas.AssuranceDashboardOut(**await service.build_dashboard())
