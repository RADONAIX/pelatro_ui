...
_LEGACY: tuple[ActionSpec, ...] = (
    ...
    # The operator's own registry, mirrored from `legacy.ACTION_SPECS` — the
    # parity test keeps codes, stages and parameter contracts identical.
    _A("SET_RATING_UNIT", "Set Rating Unit", "BASE_CHARGE", RuleCategory.COMMON,
       target_attribute="quantity", value_param="unit",
       params=(
           _P("unit", "Unit", ValueType.ENUM, values=_TIME_UNITS),
           _P("per_units", "Per units", ValueType.NUMBER, False,
              description="Defaults to 1. e.g. rate per 60 SECOND."),
       ),
       description="Defines the charging unit such as seconds or KB."),
    _A("SET_ROUNDING", "Set Pulse Rounding", "ROUNDING", RuleCategory.COMMON,
       target_attribute="charge", value_param=None,
       params=(
           _P("mode", "Mode", ValueType.ENUM, False, values=_ROUND_MODES),
           _P("decimals", "Decimals", ValueType.NUMBER, False),
       ),
       description="Defines ceiling, floor or nearest rounding."),
    _A("CONSUME_ALLOWANCE", "Consume Allowance", "BUNDLE", RuleCategory.COMMON,
       target_attribute="quantity", value_param="bundle_code",
       params=(
           _P("bundle_code", "Bundle", ValueType.REFERENCE, reference="bundles"),
           _P("consume_order", "Consume order", ValueType.NUMBER, False,
              description="Lower consumes first when several bundles match."),
       ),
       description="Consumes free units from a balance bucket."),
    _A("SET_FREE_QUANTITY", "Set Free Quantity", "BUNDLE", RuleCategory.COMMON,
       target_attribute="quantity", value_param="quantity",
       params=(
           _P("quantity", "Quantity", ValueType.NUMBER,
              description="Maximum free quantity, in the rate's unit."),
           _P("unit", "Unit", ValueType.ENUM, False, values=_TIME_UNITS),
       ),
       description="Defines the maximum free quantity."),
    _A("APPLY_PERCENT_DISCOUNT", "Apply Percentage Discount", "DISCOUNT",
       RuleCategory.COMMON, target_attribute="charge", value_param="percentage",
       params=(_P("percentage", "Percentage", ValueType.NUMBER),),
       description="Applies a percentage discount."),
    _A("APPLY_FIXED_DISCOUNT", "Apply Fixed Discount", "DISCOUNT", RuleCategory.COMMON,
       target_attribute="charge", value_param="amount",
       params=(_P("amount", "Amount", ValueType.MONEY), _CURRENCY),
       description="Applies a fixed monetary discount."),
    _A("APPLY_PERCENT_SURCHARGE", "Apply Percentage Surcharge", "SURCHARGE",
       RuleCategory.COMMON, target_attribute="charge", value_param="percentage",
       params=(_P("percentage", "Percentage", ValueType.NUMBER),),
       description="Applies a percentage surcharge."),
    _A("APPLY_FIXED_SURCHARGE", "Apply Fixed Surcharge", "SURCHARGE", RuleCategory.COMMON,
       target_attribute="charge", value_param="amount",
       params=(_P("amount", "Amount", ValueType.MONEY), _CURRENCY),
       description="Applies a fixed monetary surcharge."),
    _A("APPLY_PERCENT_TAX", "Apply Percentage Tax", "TAX", RuleCategory.COMMON,
       target_attribute="tax", value_param="percentage",
       params=(_P("percentage", "Percentage", ValueType.NUMBER),),
       description="Applies a percentage tax."),
    _A("APPLY_FIXED_TAX", "Apply Fixed Tax", "TAX", RuleCategory.COMMON,
       target_attribute="tax", value_param="amount",
       params=(_P("amount", "Amount", ValueType.MONEY), _CURRENCY),
       description="Applies a fixed monetary tax."),
)
